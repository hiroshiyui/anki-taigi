...
- **Dependencies**: `csv`, `rexml`, `rubyzip`, `sqlite3` (managed via Bundler)
...
This runs the full pipeline: download → parse ODS → export CSV cache → extract audio → generate Anki deck (.apkg + .txt).
